...
`deductra.reasoning` may depend on `deductra.domain` but not on persistence or higher layers. `deductra.memory` owns persistence ports and adapters and may depend on domain and reasoning contracts. SQLite details remain confined to `deductra.memory.sqlite_store`. `deductra.verification` may depend on domain and reasoning contracts; solver-library imports and formulas remain confined to its backend adapters. Domain, reasoning, and memory must not import verification.
